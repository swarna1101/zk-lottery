...
contract TaikoLottery is IEntropyConsumer, Ownable, ReentrancyGuard {
    IERC20 public constant TAIKO_TOKEN =
        IERC20(0x88f6D29C94E933F7C9Abf8821B081e1804579283);
    ...
    function startNewRound(
        ...
        require(
            rounds[currentRoundId].isFinished || currentRoundId == 0,
    ...
    function buyTicket(bytes32 commitment) external nonReentrant whenNotPaused {
        ...
        require(
            TAIKO_TOKEN.transferFrom(msg.sender, address(this), TICKET_PRICE),
            "Payment failed"
        );
    ...
    function claimPrize(
        ...
        require(
            verifyZKProof(zkProof, round.winningTicketIndex, roundId, input),
            "Invalid proof"
        );
        ...
        require(
            TAIKO_TOKEN.transfer(feeRecipient, feeAmount),
            "Fee transfer failed"
        );
        require(
            TAIKO_TOKEN.transfer(msg.sender, winnerAmount),
    ...
    function verifyZKProof(
        ...
        (uint[2] memory _a, uint[2][2] memory _b, uint[2] memory _c) = abi
            .decode(zkProof, (uint[2], uint[2][2], uint[2]));
